...
- Use `track` expressions with `@for` blocks to optimize list rendering.
...
**Bootstrap First**
- Use Bootstrap 5 classes, utilities, components, and grid system as much as possible for layout, spacing, typography, and responsive design.
- Use Bootstrap Icons for all iconography instead of custom SVGs or icon libraries.
- Use Bootstrap's JavaScript components (modals, dropdowns, tooltips, etc.) instead of building custom equivalents.
- Only write custom CSS when Bootstrap does not provide the needed style (e.g., cyber theme variables, glowing effects).

"""Seed ClassificationEntry with assessment areas, OWASP Top 10, and CWE data."""
...
OWASP_TOP_10 = [
    {
        ...
        'description': (
            'Failures in enforcing access restrictions, allowing users to act '
            'outside intended permissions. Includes IDOR, privilege escalation, '
    ...
    },
    {
        ...
        'description': (
            'Failures related to cryptography that lead to sensitive data '
            'exposure. Includes weak algorithms, missing encryption, improper '
            'key management, and cleartext transmission.'
        ),
    },
    {
        'code': 'A03:2021',
        'name': 'Injection',
        'description': (
            'User-supplied data sent to an interpreter as part of a command or '
            'query without proper validation. Includes SQL, NoSQL, OS command, '
    ...
    },
    {
        ...
        'description': (
            'Missing or ineffective security controls due to flawed design. '
            'Focuses on risks from design and architectural flaws, not '
            'implementation bugs.'
        ),
    },
    {
        ...
        'description': (
            'Missing security hardening, improper permissions, unnecessary '
            'features enabled, default accounts, and overly informative error '
            'messages.'
        ),
    },
    {
        ...
        'description': (
            'Using components with known vulnerabilities, unsupported software, '
            'or failing to keep libraries, frameworks, and dependencies up to '
            'date.'
        ),
    },
    {
        ...
        'description': (
            'Weaknesses in authentication and session management including '
            'brute force, default credentials, weak passwords, and session '
            'fixation.'
        ),
    },
    {
        ...
        'description': (
            'Assumptions about software updates, critical data, and CI/CD '
            'pipelines without verifying integrity. Includes insecure '
            'deserialization.'
        ),
    },
    {
        ...
        'description': (
            'Insufficient logging, monitoring, and alerting that delays or '
            'prevents detection of breaches and active attacks.'
        ),
    },
    {
        'code': 'A10:2021',
...
def seed_classifications(apps, schema_editor):
    ClassificationEntry = apps.get_model('findings', 'ClassificationEntry')
    ...
    ClassificationEntry.objects.bulk_create(entries, ignore_conflicts=True)


def remove_classifications(apps, schema_editor):
    ClassificationEntry = apps.get_model('findings', 'ClassificationEntry')
    ClassificationEntry.objects.all().delete()


class Migration(migrations.Migration):

    dependencies = [
        ('findings', '0005_alter_finding_assessment_area_and_more'),
    ]
